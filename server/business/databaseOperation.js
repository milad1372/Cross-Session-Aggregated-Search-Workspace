...
module.exports = {
    ...
    saveGallery: async function (gallery, artwork, image, galleryDescription, user, isPrivate, query) {
        ...
        if (galleryFromDatabase != null && galleryFromDatabase != "" && galleryFromDatabase != undefined) {
            ...
            // Only push the artwork if it doesn't already exist
            if (filterArtworks.length == 0 && artwork != null && artwork != "") {
            ...
            // Optionally, include summaryOutline if provided (or default to an empty string)
            let galleryFroSavingIntoDatabase = {
                ...
                user: user,
                isPrivate: isPrivate,
                summaryOutline: ""  // initialize as empty
            };
    ...
    saveLikedArtwork: async function (artwork, user) {
        ...
        if (userFromDatabase != null && userFromDatabase != "" && userFromDatabase != undefined) {
            ...
            const filterArtworks = newArtworks.filter((artworkC) => artworkC.artworkId == artwork.artworkId);
            if (filterArtworks.length == 0) newArtworks.push(artwork);
            let newValue = { $set: { artworks: newArtworks } };
    ...
    deleteLikedArtwork: async function (artworkId, user) {
        ...
        console.log("previous ArtworkIds length: ", userFromDatabase.artworks.length);
    ...
    deleteArtworkFromGallery: async function (galleryId, artworkId, user) {
        ...
        if (galleryFromDatabase != null && galleryFromDatabase != "" && galleryFromDatabase != undefined) {
            let newArtworks = galleryFromDatabase.artworks;
            console.log('Current artworks in gallery:', newArtworks.map(a => a.artworkId));
            console.log('Artwork ID to remove:', artworkId);
            const updatedArtworks = newArtworks.filter(artwork => {
                const artworkIdInArtwork = artwork.artworkId ? String(artwork.artworkId).toLowerCase() : '';
                const artworkIdToRemove = artworkId ? String(artworkId).toLowerCase() : '';
                return artworkIdInArtwork !== artworkIdToRemove;
            });
            console.log('Artworks after deletion attempt:', updatedArtworks.map(a => a.artworkId));
            let newValue = { $set: { artworks: updatedArtworks } };
            const filter = { "_id": new ObjectId(galleryId), user: user };
            try {
                let res = await db_connect.collection("galleryCollection").findOneAndUpdate(filter, newValue, { returnDocument: "after" });
                console.log("1 gallery updated: ", res);
                return true;
            ...
            console.error('Gallery not found or invalid');
            return false;
        }
    },
    ...
    updateGallery: async function (galleryNew, user) {
        let db_connect = dbo.getDb();
        // Ensure summaryOutline is updated along with other fields
        let newValue = {
            $set: {
                gallery: galleryNew.gallery,
                ...
                summaryOutline: galleryNew.summaryOutline  // <-- added field
            }
        };
        const filter = { "_id": new ObjectId(galleryNew._id) };
        try {
            let res = await db_connect.collection("galleryCollection").findOneAndUpdate(filter, newValue, { returnDocument: "after" });
            console.log("1 gallery updated: ", res);
            return true;
    ...
    getGalleries: async function (user) {
        let db_connect = dbo.getDb();
        let galleries = [];
        galleries = await db_connect.collection('galleryCollection').find({ user: user }).toArray();
        return galleries;
    },
    ...
    getLikedArtworksForUser: async function (user) {
        let db_connect = dbo.getDb();
        let likedArtworks = [];
        likedArtworks = await db_connect.collection('userLikedArtworksCollection').find({ user: user }).toArray();
    ...
    updateLog: async function (log) {
        let db_connect = dbo.getDb();
        const filter = { "_id": new ObjectId(log._id) };
