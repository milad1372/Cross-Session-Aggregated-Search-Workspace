...
module.exports = {
    ...
    retrieveDocumentsFromEuropeanaAPI: async function (searchInput, filterQuery, pageNumber, response) {
        ...
        let targetURI = Base_URL_For_Europeana + `profile=facets&sort=score+desc&query=${searchInput}&rows=${Max_Records}&start=${startRecord}&wskey=${Api_Key_EUROP}`;
    ...
    retrieveDocumentsFromWikipedia: async function(searchInput, pageNumber = 1) {
        ...
        try {
            // 1. Retrieve search results to get the list of page IDs.
            ...
            // 2. Retrieve page details using both the extracts and pageimages properties.
            const detailsParams = {
                action: "query",
                pageids: pageIds,
                prop: "pageimages|extracts",   // Request both image and full text extract.
                pithumbsize: 100,
                explaintext: true,             // Request plain text (no HTML).
                exlimit: "max",                // Ensure extracts are returned for all pages.
                redirects: 1,                  // Follow redirects if any.
                format: "json"
            };
    
            const detailsResponse = await axios.get(BASE_URL_FOR_WIKIPEDIA, { params: detailsParams });
            const pages = detailsResponse.data.query.pages;
    
            // 3. Map each search result to your desired structure.
            const transformedItems = searchResults.map(result => {
                ...
                // Use the full extract if available; otherwise, fallback to the cleaned snippet.
                const fullExtract = (page.extract && page.extract.trim())
                    ? page.extract
                ...
                return {
                    ...
                    dcCreator: ["Wikipedia"],          // Assuming Wikipedia as the creator.
                    type: "WIKI",                      // Static value indicating the type of document.
                    edmPreview: [imageUrl],            // Image URL from Wikipedia.
                    dcDescription: [fullExtract]       // The full text extract (or fallback) for the article.
                };
            });
    ...
    retrieveSummary: async function (highlightedKeys, europeanaText, wikipediaText, libraryText, selectedPassage) {
        try {
          // Adjust the URL if necessary to match your server's configuration.
